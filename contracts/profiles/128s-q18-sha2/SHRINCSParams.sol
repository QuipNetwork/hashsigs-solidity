...
/// @title SHRINCSParams (128s-q18-sha2 profile)
/// @notice Compile-time SHRINCS/SPHINCS parameter tuple for the
/// 128s-q18-sha2 profile: the SHA-256 twin of the 128s-q18 profile.
/// Every crypto constant is IDENTICAL to
/// contracts/profiles/128s-q18/SHRINCSParams.sol; only PROFILE_NAME
/// (and PROFILE_ID) carries the suite-qualified "-sha2" suffix.
/// Selected by the `shrincs-profile/` remapping paired with the sha2
/// `shrincs-hash/` remapping (foundry.toml [profile.128s-q18-sha2]).
/// @dev This directory is a deliberate duplicate of the 128s-q18
...
/// non-identity constant here to the documented 128s-q18 values under
/// the 128s-q18-sha2 profile (which CI runs), so a divergence on any
/// crypto constant fails CI. q18 fixes STATELESS_SIGNATURE_LIMIT =
/// 2^18 (= the h = 18 hypertree leaf count). Node values are truncated
/// to HASH_LEN = 16 bytes, high-aligned in a 32-byte slot (HASH_MASK,
...
library SHRINCSParams {
    ...
    // "shrincs-128s-q18-keccak" twin.
    string internal constant PROFILE_NAME = "shrincs-128s-q18-sha2";
