...
/// @title SHRINCSParams (256s-sha2 profile)
/// @notice Compile-time SHRINCS/SPHINCS parameter tuple for the 256s-sha2
/// profile: the SHA-256 twin of the 256s profile. Every crypto constant is
/// IDENTICAL to contracts/profiles/256s/SHRINCSParams.sol; only PROFILE_NAME
/// (and PROFILE_ID) carries the suite-qualified "-sha2" suffix. Selected
/// by the `shrincs-profile/` remapping paired with the sha2 `shrincs-hash/`
/// remapping (foundry.toml [profile.256s-sha2]).
/// @dev This directory is a deliberate duplicate of the 256s params. The
...
library SHRINCSParams {
    ...
    // profile's commitments from the keccak "shrincs-256s-keccak" twin.
    string internal constant PROFILE_NAME = "shrincs-256s-sha2";
