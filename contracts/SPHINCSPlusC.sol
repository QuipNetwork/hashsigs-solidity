...
/// the caller (or the SHRINCS adapter delegating here) owns the bundle and
/// commitment binding.
///
/// Revert model (same as the SHRINCS adapter). A malformed key or envelope
...
/// re-materialization bridge, since SPHINCSPlusCCore takes calldata structs
...
abstract contract SPHINCSPlusC is IERC7913SignatureVerifier {
    ...
    function verify(
        ...
        // re-materializes them as calldata for SPHINCSPlusCCore.
    ...
    /// re-encodes the validated memory data into calldata (SPHINCSPlusCCore
