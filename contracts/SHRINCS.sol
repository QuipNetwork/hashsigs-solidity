...
library SHRINCS {
    ...
    // 1. Validate the current key bundle, rotation context, and next stateful key payload.
    // 2. Recompute the next stateful-bundle commitment and require it to match the declared commitment.
    ...
    function rotateStatefulViaStateless(
        ...
        {
            // Decode the fixed-width stateful key to check operational limits such as maxSignatures.
        ...
        // Reject any mismatch between the declared and recomputed next installed-key commitment.
    ...
    // statelessRotate: Authorize replacing the full SHRINCS bundle via a stateless recovery signature.
    // 1. Validate the current key bundle, rotation context, and full next-key payload.
    // 2. Recompute the next full-bundle commitment and require it to match the declared commitment.
    ...
    function statelessRotate(
        ...
        {
            // Decode the replacement stateful key to reject unusable zero-budget keys.
            (ShrincsTypes.StatefulPublicKey memory decodedNextStatefulKey, bool ok) =
    ...
    // verifyStatefulUncheckedMessage: Verify a stateful signature after the caller has already
