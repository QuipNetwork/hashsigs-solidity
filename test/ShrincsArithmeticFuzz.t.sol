...
/// ShrincsUtils.readBits32/64 document, so the fuzz respects the caller
...
contract ShrincsArithmeticFuzzTest is Test {
    ...
    function testFuzz_readBits32(
        ...
        uint32 got = ShrincsUtils.readBits32(buffer, startBit, bitLen);
    ...
    function testFuzz_readBits64(
        ...
        uint64 got = ShrincsUtils.readBits64(buffer, startBit, bitLen);
        uint256 want = _referenceBits(buffer, startBit, bitLen);
    ...
    function testFuzz_readBits32IgnoresSlack(
        ...
        uint32 fromClean = ShrincsUtils.readBits32(clean, startBit, bitLen);
        uint32 fromNoisy = ShrincsUtils.readBits32(noisy, startBit, bitLen);
        ...
        pure
    {
        ...
        bytes memory digest = abi.encodePacked(word);
        uint32 digit = ShrincsUtils.baseWDigit(CHAIN_BASE_256, digest, index);
