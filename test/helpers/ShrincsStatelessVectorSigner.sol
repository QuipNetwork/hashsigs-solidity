...
/// @notice TEST-ONLY staged stateless SHRINCS signer for on-demand vector generation.
/// @dev This contract is intentionally placed under `test/helpers` so it does not
/// enter the production Solidity surface. It keeps the exact production math and
/// final signature layout, but splits stateless signing into storage-backed phases.
contract ShrincsStatelessVectorSigner {
    ...
    function beginSession(
        ...
        sessionId = keccak256(abi.encodePacked(address(this), msg.sender, nextSessionNonce));
        unchecked {
            ++nextSessionNonce;
        }
        ...
        bytes32 randomizer = keccak256(abi.encodePacked("fors-randomizer", signingKey.statelessPrfSeed, message));
    ...
    function stepFors(bytes32 sessionId, uint32 maxTrees) external returns (uint32 processed, bool done) {
        Session storage session = sessions[sessionId];
        ...
        uint32 signedTrees = ShrincsTypes.NUM_FORS_TREES - 1;
        while (processed < maxTrees && session.nextForsTree < signedTrees) {
            ...
            uint32 leaf = readBits32Memory(
                session.forsDigest, uint256(forsTree) * ShrincsTypes.FORS_TREE_HEIGHT, ShrincsTypes.FORS_TREE_HEIGHT
            );
            ...
            ShrincsTypes.ForsEntry storage entry = session.signature.fors.entries.push();
    ...
    function finalizeFors(bytes32 sessionId) external returns (bytes32 forsRoot) {
        Session storage session = sessions[sessionId];
        require(session.active, "unknown session");
        require(session.forsPrepared, "fors not prepared");
        require(!session.forsFinalized, "fors finalized");
        require(session.nextForsTree == ShrincsTypes.NUM_FORS_TREES - 1, "fors incomplete");
        ...
        forsRoot = keccak256(abi.encodePacked("fors-pk", session.signingKey.pkSeed, roots));
    ...
    function stepHypertree(bytes32 sessionId, uint32 maxLayers) external returns (uint32 processed, bool done) {
        Session storage session = sessions[sessionId];
        require(session.active, "unknown session");
    ...
    function finalizeSignature(bytes32 sessionId) external returns (bytes memory encodedSignature) {
        ...
        require(session.active, "unknown session");
        require(session.forsFinalized, "fors not finalized");
        require(session.nextHypertreeLayer == NUM_HYPERTREE_LAYERS, "hypertree incomplete");
        session.signatureFinalized = true;
        encodedSignature = abi.encode(copyStatelessSignature(session.signature));
    }

    function sessionPublicKey(bytes32 sessionId) external view returns (ShrincsTypes.PublicKey memory publicKey) {
        Session storage session = sessions[sessionId];
        require(session.active, "unknown session");
        publicKey = copyPublicKey(session.publicKey);
    }

    function sessionMessage(bytes32 sessionId) external view returns (bytes memory message) {
        Session storage session = sessions[sessionId];
        require(session.active, "unknown session");
    ...
    function copyStatelessSignature(ShrincsTypes.StatelessSignature storage signature)
        internal
        view
        returns (ShrincsTypes.StatelessSignature memory out)
    {
        out.fors.randomizer = signature.fors.randomizer;
        out.fors.counter = signature.fors.counter;
        out.fors.entries = new ShrincsTypes.ForsEntry[](signature.fors.entries.length);
        for (uint256 i = 0; i < signature.fors.entries.length;) {
            out.fors.entries[i].secretLeaf = signature.fors.entries[i].secretLeaf;
            out.fors.entries[i].authPath = new bytes[](signature.fors.entries[i].authPath.length);
            for (uint256 j = 0; j < signature.fors.entries[i].authPath.length;) {
                out.fors.entries[i].authPath[j] = signature.fors.entries[i].authPath[j];
                unchecked {
                    ++j;
                }
            }
        ...
        out.hypertree = new ShrincsTypes.HypertreeLayerSignature[](signature.hypertree.length);
        for (uint256 i = 0; i < signature.hypertree.length;) {
            ...
            out.hypertree[i].wotsCSignature.randomizer = signature.hypertree[i].wotsCSignature.randomizer;
            out.hypertree[i].wotsCSignature.counter = signature.hypertree[i].wotsCSignature.counter;
            out.hypertree[i].wotsCSignature.chains = new bytes[](signature.hypertree[i].wotsCSignature.chains.length);
            for (uint256 j = 0; j < signature.hypertree[i].wotsCSignature.chains.length;) {
                out.hypertree[i].wotsCSignature.chains[j] = signature.hypertree[i].wotsCSignature.chains[j];
                unchecked {
                    ++j;
                }
            }
            out.hypertree[i].authPath = new bytes[](signature.hypertree[i].authPath.length);
            for (uint256 j = 0; j < signature.hypertree[i].authPath.length;) {
                out.hypertree[i].authPath[j] = signature.hypertree[i].authPath[j];
                unchecked {
                    ++j;
                }
            }
        ...
        returns (bytes memory digest, uint64 treeIndex, uint32 leafIndex)
    {
        ...
        digest = forsDigestBytes(pkSeed, hypertreeRoot, randomizer, counter, message, digestBytes);
        uint256 cursor = indexBits;
        treeIndex = readBits64Memory(digest, cursor, treeBits);
        leafIndex = readBits32Memory(digest, cursor + treeBits, subtreeHeight);
    }
    ...
    function forsDigestBytes(
        ...
        bytes memory base = abi.encodePacked("fors-digest", pkSeed, hypertreeRoot, randomizer, counter, message);
        if (digestBytes <= 32) {
        ...
        while (offset < digestBytes) {
            bytes32 digestWord = keccak256(abi.encodePacked(base, blockCounter));
    ...
    function forsTreeRootAndAuthPath(
        ...
        uint32 height = ShrincsTypes.FORS_TREE_HEIGHT;
        uint256 leafCount = uint256(1) << height;
        bytes32[] memory levelNodes = new bytes32[](leafCount);
        for (uint32 index = 0; index < leafCount;) {
            levelNodes[index] = forsLeafHash(pkSeed, skSeed, treeIndex, leafIndex, forsTree, index);
            unchecked {
                ++index;
            }
        }
    ...
    function forsLeafHash(
        ...
        uint64 treeLeaf = (uint64(forsTree) << ShrincsTypes.FORS_TREE_HEIGHT) + uint64(leaf);
        bytes32 addressWord = forsAddressWord(treeIndex, leafIndex, 0, treeLeaf);
        return keccak256(abi.encodePacked("fors-leaf", pkSeed, addressWord, secret));
    }
    ...
    function signStatelessWotsC(
        ...
        uint32 keypair,
        bytes32 message
    ) internal pure returns (bytes32 randomizer, uint32 counter, bytes32[] memory chains, bool ok) {
        randomizer = keccak256(abi.encodePacked("wots-c-randomizer", statelessPrfSeed, message));
    ...
    function buildStatelessWotsChains(
        ...
        chains = new bytes32[](ShrincsTypes.NUM_WOTS_CHAINS);
        for (uint32 chain = 0; chain < ShrincsTypes.NUM_WOTS_CHAINS;) {
            ...
            chains[chain] = statelessWotsCChain(pkSeed, layer, tree, keypair, chain, secret, 0, digit);
            unchecked {
                ++chain;
            }
        }
    ...
    function readBits32Memory(bytes memory input, uint256 startBit, uint32 bitLen) internal pure returns (uint32) {
        return uint32(readBitsMemory(input, startBit, bitLen));
    }

    function readBits64Memory(bytes memory input, uint256 startBit, uint32 bitLen) internal pure returns (uint64) {
        return uint64(readBitsMemory(input, startBit, bitLen));
    }

    function readBitsMemory(bytes memory input, uint256 startBit, uint32 bitLen) internal pure returns (uint64 out) {
