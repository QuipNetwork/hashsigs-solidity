...
library ShrincsToyStatelessProfile {
    ...
    {
        bytes32 statelessSkSeed =
            derive32("toy-stateless-sk-seed", seedMaterial, "");
        bytes32 statelessPrfSeed =
            derive32("toy-stateless-prf-seed", seedMaterial, "");
        ...
        returns (StatelessSignature memory signature, bool ok)
    {
        ...
        uint64 treeIndex;
        uint32 leafIndex;
        (forsRoot, forsSignature, treeIndex, leafIndex, ok) =
            signFors(signingKey, message);
        if (!ok) return (signature, false);

        HypertreeLayerSignature[] memory layers;
        (layers, ok) =
            signHypertree(signingKey, forsRoot, treeIndex, leafIndex);
        if (!ok) return (signature, false);

        signature =
            StatelessSignature({fors: forsSignature, hypertree: layers});
    ...
    function verify(
        ...
        uint32 leafIndex;
        bool ok;
        (forsRoot, treeIndex, leafIndex, ok) =
            verifyFors(publicKey, message, signature.fors);
        if (!ok) return false;
        return verifyHypertree(
            publicKey, forsRoot, treeIndex, leafIndex, signature.hypertree
        );
    }
    ...
    function signFors(SigningKey memory signingKey, bytes memory message)
        ...
        returns (
            bytes32 forsRoot,
            ForsSignature memory signature,
            uint64 treeIndex,
            uint32 leafIndex,
            bool ok
        )
    {
        bytes32 randomizer = keccak256(
            abi.encodePacked(
                "toy-fors-randomizer", signingKey.statelessPrfSeed, message
            )
        );
        for (uint32 counter = 0; counter < MAX_GRIND_COUNTER;) {
            ...
            (digest, digestTreeIndex, digestLeafIndex) = forsDigest(
                signingKey.pkSeed,
            ...
            if (
                readBits32(
                        digest,
                        SIGNED_FORS_TREES * FORS_TREE_HEIGHT,
                        FORS_TREE_HEIGHT
                    ) != 0
            ) {
                unchecked {
            ...
            forsRoot = keccak256(
                abi.encodePacked("toy-fors-pk", signingKey.pkSeed, roots)
            );
            return
                (forsRoot, signature, digestTreeIndex, digestLeafIndex, true);
        }
    ...
    {
        if (signature.entries.length != SIGNED_FORS_TREES) {
        ...
        bytes memory roots = new bytes(SIGNED_FORS_TREES * 32);
        for (uint32 forsTree = 0; forsTree < SIGNED_FORS_TREES;) {
            uint32 entryLeaf = readBits32(
                digest,
                uint256(forsTree) * FORS_TREE_HEIGHT,
                FORS_TREE_HEIGHT
            );
            bytes32 root = forsEntryRoot(
                publicKey.pkSeed,
                treeIndex,
                leafIndex,
                forsTree,
                entryLeaf,
                signature.entries[forsTree]
            );
            setSlice32(roots, root, uint256(forsTree) * 32);
            unchecked {
                ++forsTree;
        ...
        forsRoot = keccak256(
            abi.encodePacked("toy-fors-pk", publicKey.pkSeed, roots)
        );
        return (forsRoot, treeIndex, leafIndex, true);
    }

    function signHypertree(
        ...
        returns (HypertreeLayerSignature[] memory layers, bool ok)
    {
        bytes32[NUM_HYPERTREE_LAYERS] memory layerSeeds =
            hypertreeLayerSeeds(signingKey.statelessSkSeed);
        ...
        for (uint32 layer = 0; layer < NUM_HYPERTREE_LAYERS;) {
            bytes32 leafSeed = keccak256(
                abi.encodePacked(
                    "toy-hypertree-leaf-seed", layerSeeds[layer], tree, leaf
                )
            );
            bytes32 skSeed = keccak256(
                abi.encodePacked("toy-hypertree-wots-sk-seed", leafSeed)
            );
            bytes32 pkHash =
                wotsPublicKey(signingKey.pkSeed, skSeed, layer, tree, leaf);
            WotsSignature memory wotsSignature;
            (wotsSignature, ok) = signWots(
                signingKey.pkSeed,
                skSeed,
                signingKey.statelessPrfSeed,
                pkHash,
                layer,
                tree,
                leaf,
            ...
            bytes32[] memory authPath = hypertreeAuthPath(
                signingKey.pkSeed, layerSeeds[layer], layer, tree, leaf
            );
    ...
    function signWots(
        ...
        bytes32 randomizer = keccak256(
            abi.encodePacked("toy-wots-randomizer", prfSeed, message)
        );
        for (uint32 counter = 0; counter < MAX_GRIND_COUNTER;) {
            ...
            if (digitSum == WOTS_TARGET_SUM) {
                signature = WotsSignature({
                    randomizer: randomizer, counter: counter, chains: chains
                });
                return (signature, true);
            }
            unchecked {
    ...
    function verifyWots(
        ...
        for (uint32 chain = 0; chain < NUM_WOTS_CHAINS;) {
            ...
            bytes32 endpoint = wotsChain(
                ...
                keypair,
                chain,
                signature.chains[chain],
                digit,
                WOTS_CHAIN_LEN - 1 - digit
            );
        ...
        returns (bytes memory digest, uint64 treeIndex, uint32 leafIndex)
    {
        ...
        uint256 digestBytes =
            (uint256(indexBits) + uint256(HYPERTREE_HEIGHT) + 7) / 8;
        digest = new bytes(digestBytes);
        bytes32 digestWord = keccak256(
            abi.encodePacked(
                "toy-fors-digest",
                pkSeed,
                hypertreeRoot,
    ...
    function forsTreeRootAndAuthPath(
        ...
        for (uint32 i = 0; i < leafCount;) {
            levelNodes[i] = forsLeafHash(
                pkSeed, skSeed, treeIndex, leafIndex, forsTree, i
            );
            unchecked {
                ++i;
            }
    ...
    function forsEntryRoot(
        ...
        bytes32 addressWord = forsAddressWord(
            ...
            leafIndex,
            0,
            (uint64(forsTree) << FORS_TREE_HEIGHT) + uint64(entryLeaf)
        );
        node = keccak256(
            abi.encodePacked(
                "toy-fors-leaf", pkSeed, addressWord, entry.secretLeaf
            )
        );
        ...
        for (uint32 level = 0; level < FORS_TREE_HEIGHT;) {
            ...
            node = keccak256(
                abi.encodePacked(
                    "toy-fors-node", pkSeed, parentWord, left, right
                )
            );
            index >>= 1;
            unchecked {
        ...
        returns (bytes32)
    {
        ...
        return
            hypertreeVirtualNode(
                pkSeed, layerSeeds[1], 1, 0, SUBTREE_HEIGHT, 0
            );
    }
    ...
    function hypertreeVirtualNode(
        ...
        bytes32 right = hypertreeVirtualNode(
            pkSeed, layerSeed, layer, tree, height - 1, (index << 1) | 1
        );
        return keccak256(
            abi.encodePacked(
                "toy-hypertree-node",
    ...
    function hypertreeLeaf(
        ...
        bytes32 leafSeed = keccak256(
            abi.encodePacked(
                "toy-hypertree-leaf-seed", layerSeed, tree, leaf
            )
        );
        bytes32 skSeed = keccak256(
            abi.encodePacked("toy-hypertree-wots-sk-seed", leafSeed)
    ...
    function wotsSecret(bytes32 skSeed, uint32 chain)
        internal
        pure
        returns (bytes32)
    {
        ...
        for (uint32 step = start; step < start + steps;) {
            out = keccak256(
                abi.encodePacked(
                    "toy-wots-chain",
                    pkSeed,
                    wotsAddressWord(layer, tree, keypair, chain, step),
                    out
                )
            );
            unchecked {
        ...
        return keccak256(
            abi.encodePacked(
                ...
                forsAddressWord(
                    ...
                    leafIndex,
                    0,
                    (uint64(forsTree) << FORS_TREE_HEIGHT) + uint64(leaf)
                )
            )
        );
    }
    ...
    function forsLeafHash(
        ...
        return keccak256(
            abi.encodePacked("toy-fors-leaf", pkSeed, addressWord, secret)
        );
    }
...
contract ShrincsToyStatelessProfileTest is Test {
    ...
    function testToyProfileStatelessSignAndVerify() public view {
        ...
        assertTrue(
            ...
            "toy stateless signature must verify"
        );
    }
